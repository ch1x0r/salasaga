 ...
 * $Id$
 *
 * Salasaga: Function called when the x offset finish field in the timeline widget is edited 
 * 
...
void timeline_edited_x_offset_finish(GtkCellRendererText *selection, gchar *row, gchar *new_value, gpointer data)
{
	...
	switch (layer_data->object_type)
	{
		...
		case TYPE_GDK_PIXBUF:
			...
			{
				if (layer_data->x_offset_finish != atoi(new_value))
				{
					// If the user attempted to change the background layer offset, we give them feedback not to
					display_warning("Error ED351: Background layers must remain at 0.");
				}
				return;
			}
		...
		case TYPE_TEXT:
			...
			{
				display_warning("Error ED353: There was something wrong with the new x offset finish value.  Ignoring it.");
				return;
			} else
			{
				// Apply the change
				layer_data->x_offset_finish = *validated_guint;
				g_free(validated_guint);
			}
			break;

		default:
			display_warning("Error ED30: Unknown layer type\n");
	}
