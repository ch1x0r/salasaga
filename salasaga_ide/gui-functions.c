 ...
 *
 * Revision 1.164  2006/04/13 15:27:55  jc
 * + Included a sound beep for some errors and warnings.
 * + Added further error messages.
 *
 * Revision 1.163  2006/04/11 09:50:54  jc
 * + Updated the thumbnail redrawing function to work on whichever slide it is asked to, rather than hard coded to always use the slide currently selected in the GUI.
 * + Updated the calls to the thumbnail drawing function to use its new parameter.
 * + Added working code to crop the entire project.
 *
 * Revision 1.162  2006/04/11 06:30:00  jc
 * Added working code for cropping image layers.
 *
 * Revision 1.161  2006/04/10 08:20:13  jc
 * + Added a background color swatch to the New Project dialog.
 * + Fixed the New Project dialog so it updates the timeline and working area after creating the new slide.
 *
 * Revision 1.160  2006/04/10 08:00:57  jc
 * + Empty layers display correctly in the working area now.
 * + Inserted slides appear after the presently selected one.
 *
 * Revision 1.159  2006/04/10 06:55:06  jc
 * + slide_delete() now frees the memory for each of its layers.
 * + Added working code to slide_move_up() and slide_move_down().
 *
 * Revision 1.158  2006/04/10 05:45:26  jc
 * Started adding working code to the slide_delete function to free unused memory.
 *
 * Revision 1.157  2006/04/09 12:14:50  jc
 ...
 *
 * Revision 1.156  2006/04/08 07:28:36  jc
 * + Added a new function, refresh_film_strip, for putting the slide thumbnails into the film strip area.
 * + Updated the screenshot import and slide inserting functions to use it.
 *
 * Revision 1.155  2006/04/07 00:59:03  jc
 * Moved the definition of external variables into its own common header file.
 *
 * Revision 1.154  2006/04/07 00:54:36  jc
 * Moved much of the film strip setup code into create_film_strip().
 *
 * Revision 1.153  2006/04/06 03:34:22  jc
 * + Moved slide_insert(), slide_delete(), slide_move_up(), and slide_move_down() here.
 * + Started adding working code to slide_insert().  Needs more work.
 *
 * Revision 1.152  2006/04/06 01:19:58  jc
 * Simplified the greying out and re-enabling of menu items.
 *
 * Revision 1.151  2006/04/05 17:00:14  jc
 * Updated externs.
 *
 * Revision 1.150  2006/03/31 05:06:47  jc
 * + Updated externs.
 * + Changed text display font family to Bitstream Vera Sans.
 *
 * Revision 1.149  2006/03/27 05:37:09  jc
 * Added working code for the layer_move_down function.  It could probably be a lot more efficient.
 *
 * Revision 1.148  2006/03/27 05:24:08  jc
 * Added working code for the layer_move_up function.  It could probably be a lot more efficient.
 *
 * Revision 1.147  2006/03/26 23:40:53  jc
 * + Added ming include.
 * + Removed timeline.h include.
 * + Started fleshing out the flash export code.
 *
 * Revision 1.146  2006/03/26 05:59:09  jc
 ...
 *
 * Revision 1.145  2006/03/25 05:20:41  jc
 * Changed all the numeric entry fields to use GtkSpinButton widgets.
 *
 * Revision 1.144  2006/03/24 14:10:50  jc
 * Fixed a small bug in the layer_delete function, causing the layer list pointer to not be updated properly.
 *
 * Revision 1.143  2006/03/24 05:30:30  jc
 * Added the draw_bounding_box() function - not presently used - and made other small updates.
 *
 * Revision 1.142  2006/03/23 07:15:51  jc
 * Adding further collision detection callbacks and code.
 *
 * Revision 1.141  2006/03/22 13:50:48  jc
 * Updated to store rendered width and height info in the text layer data structure.
 *
 * Revision 1.140  2006/03/22 07:14:24  jc
 * + Updated the draw_workspace function to use the new backing store and drawing area.
 * + Renamed the compress_image functions for clarity, so we no longer use them as GtkImages.
 * + Small cleanups of other code.
 *
 * Revision 1.139  2006/03/21 21:29:10  jc
 * + Updated the default background color to use a GdkColor.
 * + Updated the preferences dialog to use folder selection buttons.
 *
 * Revision 1.138  2006/03/21 13:58:58  jc
 * + Continued the simplification of code for text inputs.
 * + Fixed a bug where text layers weren't always being drawn correctly due to pixmap/pixbuf sizing being hard coded wrongly.
 *
 * Revision 1.137  2006/03/21 12:18:01  jc
 * + Improved File New function to ask for project width and height.  It still needs to create the backing pixbuf.
 * + Simplified some of the data entry code.
 * + Added start and finish frame entries to the highlight and image layer creation dialogs.
 *
 * Revision 1.136  2006/03/21 08:10:07  jc
 ...
 *
 * Revision 1.135  2006/03/21 05:49:35  jc
 * Now correctly uses and displays the color of the text layer.  I wish the Pango layout functions were documented better (like, at all for things like pango_renderer_draw_layout() which turned out to be absolutely required).
 *
 * Revision 1.134  2006/03/21 05:14:04  jc
 * Converting the text layer to use a GdkColor and color picker instead of three separate component values.
 *
 * Revision 1.133  2006/03/20 07:32:49  jc
 * Fixed small bugs, so oversize highlight and text layers will still be drawn (at least reasonably) in the working area.
 *
 * Revision 1.132  2006/03/20 07:07:45  jc
 * Fixed a bug, so now the film strip area displays properly when used with a minimal amount of slides.
 *
 * Revision 1.131  2006/03/20 06:59:33  jc
 * + Image layers are now drawn in the workspace.
 * + Fixed a bug, so layers are drawn in the correct order in the workspace.
 *
 * Revision 1.130  2006/03/20 06:20:19  jc
 * Added working code for adding an image layer.  Still need to adjust the workspace drawing code to handle it though.
 *
 * Revision 1.129  2006/03/19 10:57:40  jc
 * Text layers now have a light yellow background (with black border).  Looks ok.
 *
 * Revision 1.128  2006/03/19 10:39:33  jc
 * Moved the highlight box drawing code into its own function, so it can be called by other things.
 *
 * Revision 1.127  2006/03/19 10:03:41  jc
 * Added code for rendering the text layer in the working area.  Still need to create the background layer behind it though.
 *
 * Revision 1.126  2006/03/18 15:00:31  jc
 * + Removed/commented-out Cairo code.
 * + Updated to display a proper inner and outer transparent highlight box.
 *
 * Revision 1.125  2006/03/18 05:14:20  jc
 * Highlight layers are now properly updated through a dialog box for the user.
 *
 * Revision 1.124  2006/03/18 04:36:22  jc
 * Text layers are now properly updated through a dialog box for the user.
 *
 * Revision 1.123  2006/03/18 01:37:03  jc
 * Adjusted the functions to handing starting and finishing x and y offsets.
 *
 * Revision 1.122  2006/03/17 11:02:01  jc
 * Started adding code for editing an existing layer.
 *
 * Revision 1.121  2006/03/14 10:03:54  jc
 ...
 *
 * Revision 1.120  2006/03/13 04:17:24  jc
 * Added a Save Format version number to the saved project output, to hopefully aid future backwards compatibility.
 *
 * Revision 1.119  2006/03/13 04:05:28  jc
 * + Added signal handlers to the timeline widget, for handling field edits well.
 * + Removed the file type column from the timeline widget.
 *
 * Revision 1.118  2006/03/12 10:50:34  jc
 * Fixed a typo in two field labels.
 *
 * Revision 1.117  2006/03/12 04:34:27  jc
 * Updated the compress_layers_to_image function to use the new layer_image structure.
 *
 * Revision 1.116  2006/03/12 04:14:44  jc
 * Updating the screenshot import function to use the new layer_image data type.
 *
 * Revision 1.115  2006/03/12 02:38:14  jc
 * Adding initial code to save the project preferences in the project file.
 *
 * Revision 1.114  2006/03/11 05:48:57  jc
 * Started adding libxml code to the menu_file_save function.
 *
 * Revision 1.113  2006/03/09 14:28:45  jc
 * + Began adding code for parsing in an XML .flame file.
 * + Added a empty stub backend function for saving an XML .flame file.
 *
 * Revision 1.112  2006/03/08 14:40:06  jc
 * Added the screenshots folder to the Preferences dialog.
 *
 * Revision 1.111  2006/03/07 14:19:28  jc
 ...
 *
 * Revision 1.110  2006/03/06 10:44:01  jc
 * + Began adding code to delete the layer selected in the timeline.
 * + Removed a few redraws of the timeline that weren't needed.
 *
 * Revision 1.109  2006/03/05 09:50:04  jc
 * Added saving and restoring of cairo context, as highlighting wasn't working properly without it.
 *
 * Revision 1.108  2006/03/05 09:34:38  jc
 * + Added first working code to render slide layers using cairo.
 * + Bumped up the application version number.
 *
 * Revision 1.107  2006/03/05 04:17:20  jc
 * Added initial play-around code testing out cairo functions.
 *
 * Revision 1.106  2006/03/05 02:37:02  jc
 ...
 *
 * Revision 1.105  2006/03/05 01:38:21  jc
 * Added a global variable to hold the dedicated screenshots folder.
 *
 * Revision 1.104  2006/03/04 12:26:33  jc
 * Added code to create a stub project save file.
 *
 * Revision 1.103  2006/03/04 11:52:08  jc
 * Started moving backend functions into new source backend.c and header backend.h files.
 *
 * Revision 1.102  2006/03/02 09:44:23  jc
 * Fixed a small bug, where the Save dialog box had an Open button instead.
 *
 * Revision 1.101  2006/03/02 09:43:36  jc
 * Added initial working code to the menu_file_save function, so it opens a dialog to select the folder and filename to save as.
 *
 * Revision 1.100  2006/03/01 07:56:12  jc
 * Started adding the code for the menu_file_new dialog box.
 *
 * Revision 1.99  2006/02/27 18:23:57  jc
 ...
 *
 * Revision 1.98  2006/02/27 17:06:29  jc
 * Added a dialog box to the screenshot capture code, asking for capture area information,
 *
 * Revision 1.97  2006/02/27 08:18:50  jc
 * Added working code for creating the ~/.flame-lock file.  Seems to work fine.
 *
 * Revision 1.96  2006/02/26 00:04:33  jc
 * Added initial working code to display a dialog for the File -> Open function.  No code to parse the open file written yet though.
 *
 * Revision 1.95  2006/02/25 11:59:52  jc
 * Moved the contents of the old menu_file_new function into menu_screenshots_import.
 *
 * Revision 1.94  2006/02/23 10:20:55  jc
 ...
 *
 * Revision 1.93  2006/02/21 09:58:17  jc
 * Added timeline toolbar buttons for adding a highlight layer, and cropping the project.
 *
 * Revision 1.92  2006/02/21 09:35:36  jc
 * + Fixed a bug so the top line of the timeline widget is selected when the widget is created.
 * + Reduced the number of places and times the timeline widget is destroyed and re-created.
 *
 * Revision 1.91  2006/02/20 08:27:27  jc
 * Started adding code to highlight the top layer in the timeline area of a slide.
 *
 * Revision 1.90  2006/02/20 08:10:15  jc
 * Updated the timeline list store widget so it displays the text in a text layer.
 *
 * Revision 1.89  2006/02/19 02:05:10  jc
 * Added initial working code to prompt for and store the details of the new text layer.
 *
 * Revision 1.88  2006/02/18 09:01:53  jc
 * Started adding a layer button to add a text layer.
 *
 * Revision 1.87  2006/02/18 08:35:01  jc
 * Simplified the timeline widget container, fixing a bug.
 *
 * Revision 1.86  2006/02/18 08:22:27  jc
 * Converted the timeline widget to a GtkListStore.
 *
 * Revision 1.85  2006/02/16 09:32:33  jc
 * Started converting the timeline widget to a GtkTreeView.
 *
 * Revision 1.84  2006/02/15 20:38:42  jc
 * Timeline widget now works properly again, with layer creation and destruction functions simplified.
 *
 * Revision 1.83  2006/02/14 09:49:09  jc
 ...
 *
 * Revision 1.82  2006/02/12 01:36:01  jc
 * Debugging underway so the timeline widget does not get destroyed each time the timeline is refreshed.
 *
 * Revision 1.81  2006/02/12 01:11:15  jc
 * Simplified some pointer handling code.
 * Still needs a lot of debugging.
 *
 * Revision 1.80  2006/02/12 00:52:04  jc
 * Changing how timeline widgets are used.  Something is grossly unhappy.
 *
 * Revision 1.79  2006/02/12 00:04:35  jc
 * Simplified the layer data structure.
 *
 * Revision 1.78  2006/02/09 11:05:51  jc
 * Adding code to try and force the table to remain at the top of the time line.
 *
 * Revision 1.77  2006/02/09 10:57:10  jc
 * Added a decent header to the time line table.
 *
 * Revision 1.76  2006/02/09 10:10:45  jc
 * Fixed a small bug, so the timeline will show the layers for the selected slide.
 *
 * Revision 1.75  2006/02/09 10:08:33  jc
 * Added some (crappy) code to clear the widgets out of the time line table, to allow a cleaner redraw.
 *
 * Revision 1.74  2006/02/09 10:01:11  jc
 * Started adding code to add a new layer when the user clicks the Add button in the time line toolbar.
 *
 * Revision 1.73  2006/02/08 09:01:11  jc
 * Tweaked so imported screenshots are automatically named Background for their layer.
 *
 * Revision 1.72  2006/02/08 08:56:25  jc
 * Added a toolbar to the time line area.
 *
 * Revision 1.71  2006/02/07 10:58:34  jc
 * Added very initial code show the layer information in the time line.
 * Still needs a lot of work.
 *
 * Revision 1.70  2006/02/06 08:44:47  jc
 * Started creating the draw_timeline() function, which will display the layers of the selected slide in the time line area.
 *
 * Revision 1.69  2006/02/05 06:59:54  jc
 ...
 *
 * Revision 1.68  2006/02/05 03:48:24  jc
 * Added initial working code to make the new Preferences values effective.
 *
 * Revision 1.67  2006/02/05 02:58:27  jc
 * Divided the Preferences dialog into two notebook tabs, General and Project.
 *
 * Revision 1.66  2006/02/05 01:33:53  jc
 ...
 *
 * Revision 1.65  2006/02/04 15:37:54  jc
 * Added fields for default output width and height to the Preferences dialog.
 *
 * Revision 1.64  2006/02/04 15:20:15  jc
 * Removed output width and output height from the Preferences dialog, as they're already available through the main gui window.
 *
 * Revision 1.63  2006/02/04 13:12:39  jc
 * Added Project Path, Output Path, Output Width, Output Height, and Output Quality to the Preferences dialog.
 *
 * Revision 1.62  2006/02/04 11:16:04  jc
 * Added icon height, project name, and slide length to the Preferences dialog.
 *
 * Revision 1.61  2006/02/04 07:02:13  jc
 * Added the preview width to the preferences dialog, and set the label alignment to the left for all of the fields.
 *
 * Revision 1.60  2006/02/04 06:41:41  jc
 * Began adding working code for the Preferences dialog.
 *
 * Revision 1.59  2006/02/02 09:31:26  jc
 ...
 *
 * Revision 1.58  2006/01/31 10:46:06  jc
 * Updated to use an ItemFactory for the menu creation, rather than the initial manual approach.
 *
 * Revision 1.57  2006/01/30 08:23:38  jc
 * Replaced more code with our custom error handler.
 *
 * Revision 1.56  2006/01/30 08:21:00  jc
 * Added an initial (untested) error handling function.
 *
 * Revision 1.55  2006/01/29 10:16:52  jc
 * + Added a toolbar icon to export as an SVG.
 * + Small cleanups.
 *
 * Revision 1.54  2006/01/29 06:19:36  jc
 * + Added code to free temporarily allocated gdk pixbufs.
 * + Added code to display status bar messages during the export.
 * + Chose a different scaling algorithm for gdk pixbuf scaling that gives a better end result and processes quicker.
 *
 * Revision 1.53  2006/01/29 05:49:10  jc
 * Added initial working code to correctly output the starting and finishing times in the output SVG file.
 *
 * Revision 1.52  2006/01/29 05:35:12  jc
 * Added initial working code that saves the component png files.
 *
 * Revision 1.51  2006/01/29 05:09:20  jc
 ...
 *
 * Revision 1.50  2006/01/29 03:33:02  jc
 * Added the closing SVG tag to the output file.
 *
 * Revision 1.49  2006/01/29 03:24:18  jc
 * Added code to write the header to the output SVG file.
 *
 * Revision 1.48  2006/01/29 02:42:29  jc
 * It looks like memory is only allocated to a GError when an error occurs, so there was no need to free memory if no error has occured.  Much nicer. :)
 *
 * Revision 1.47  2006/01/29 02:39:41  jc
 * Updated to use the GIOChannel functions rather than g_open(), etc.
 *
 * Revision 1.46  2006/01/28 14:48:43  jc
 * Bug fixed the previous commit's code.
 *
 * Revision 1.45  2006/01/28 14:35:49  jc
 * Adding stub code to create the output SVG animation file.
 *
 * Revision 1.44  2006/01/28 12:32:31  jc
 * Turned the default slide length into a variable, rather than having it hard coded.
 *
 * Revision 1.43  2006/01/28 08:55:11  jc
 ...
 *
 * Revision 1.38  2006/01/25 11:16:53  jc
 * Moved the configuration variables into two types, application level ones (including defaults), and project level ones.
 *
 * Revision 1.37  2006/01/24 12:05:54  jc
 * Added a comment showing an initial working way of doing AVG animation.
 *
 * Revision 1.36  2006/01/24 10:11:58  jc
 * Added a stub function for exporting flash animation as well.
 *
 * Revision 1.35  2006/01/23 09:24:26  jc
 * Added a blank stub function for exporting an SVG Animation.
 *
 * Revision 1.34  2006/01/22 06:24:49  jc
 * Started updating things so the File -> New menu option can be used more than once.
 *
 * Revision 1.33  2006/01/22 06:17:51  jc
 * Fixed the previous bug, so that if the working area already has an image in it, then remove the old old before adding the new one.
 *
 * Revision 1.32  2006/01/22 06:00:24  jc
 * Added working code to refresh the drawing area when a slide is clicked on.
 * It's exposed a bug in some other code though, which needs fixing first.
 *
 * Revision 1.31  2006/01/22 04:27:27  jc
 * Added a 1 pixel black border to the slide displayed in the working area.
 *
 * Revision 1.30  2006/01/22 03:58:18  jc
 * Fixed menu_file_new() so it allocates memory dynamically.
 *
 * Revision 1.29  2006/01/22 03:40:36  jc
 * Added initial working code to put the selected layer onscreen.
 *
 * Revision 1.28  2006/01/22 02:05:32  jc
 * + Added code to initialise the main pixbuf area in compress_layers_to_image().
 * + Reverted the loading of the screenshots back to GdkPixmaps rather than GtkImages, because they're easier to work with (thus far).
 *
 * Revision 1.27  2006/01/22 01:18:50  jc
 * Added initial working code for scaling and displaying the screenshot into the working area.
 *
 * Revision 1.26  2006/01/22 00:18:19  jc
 * Started getting things ready for the code to compress the layers to an image.
 *
 * Revision 1.25  2006/01/21 08:27:04  jc
 * Started rearranging code into a stub function for creating an image from a slide's layers.
 *
 * Revision 1.24  2006/01/21 08:16:52  jc
 * Moved the code for creating an aspect frame into the working_area creation function.
 *
 * Revision 1.23  2006/01/21 08:10:36  jc
 * Started adding code to display the current slide in the drawing area.
 *
 * Revision 1.22  2006/01/21 07:04:39  jc
 * Updated to reload the screenshots at full size for the slide layers.
 *
 * Revision 1.21  2006/01/21 06:55:46  jc
 * Moved around the code to create an initial layer for each slide, and use the loaded screenshots for that background layer.
 *
 * Revision 1.20  2006/01/21 05:13:05  jc
 * Moved the typedef and enum definitions into a common file.
 *
 * Revision 1.19  2006/01/21 05:04:55  jc
 * Added an initial enum defining known object types.
 *
 * Revision 1.18  2006/01/21 01:28:22  jc
 * Very small optimisation.
 *
 * Revision 1.17  2006/01/21 01:25:02  jc
 * Adding the foundational structures and code for using layers.
 *
 * Revision 1.16  2006/01/20 12:59:43  jc
 * Removed the message dialog showing the project path.
 * Removed some crufty code that wasn't doing anything real.
 *
 * Revision 1.15  2006/01/20 12:49:30  jc
 * Updated to redraw the screen as the images are loaded, to show loading progress.
 *
 * Revision 1.14  2006/01/20 12:13:57  jc
 * Moved the image thumbnails and event boxes into a global GList.
 *
 * Revision 1.13  2006/01/19 12:03:39  jc
 * Put the thumbnail images into an event box, so they can receive events, and added code for displaying a tooltip for them showing their slide number.
 *
 * Revision 1.12  2006/01/19 11:39:27  jc
 * Added some comments.
 *
 * Revision 1.11  2006/01/19 11:38:02  jc
 * Updated the thumbnail loading code to give status bar messages about the number of images loaded.
 *
 * Revision 1.10  2006/01/15 09:32:02  jc
 * Added a separator between the thumbnail previews, making things that little bit nicer. :)
 *
 * Revision 1.9  2006/01/15 09:24:53  jc
 ...
 *
 * Revision 1.4  2006/01/15 04:42:34  jc
...
GtkWidget *construct_timeline_widget(slide *slide_data)
{
	...
	GtkCellRenderer		*timeline_renderer_text_final;
	GtkCellRenderer		*timeline_renderer_text_x_offset;
	GtkCellRenderer		*timeline_renderer_text_y_offset;
	GtkCellRenderer		*timeline_renderer_text_text;
	...
	GtkTreeViewColumn	*timeline_column_final;
	GtkTreeViewColumn	*timeline_column_x_off;
	GtkTreeViewColumn	*timeline_column_y_off;
	GtkTreeViewColumn	*timeline_column_text;
	...
	timeline_renderer_text_final = gtk_cell_renderer_text_new();
	timeline_renderer_text_x_offset = gtk_cell_renderer_text_new();
	timeline_renderer_text_y_offset = gtk_cell_renderer_text_new();
	timeline_renderer_text_text = gtk_cell_renderer_text_new();
	...
	// Hook a signal handler to the final column renderer
	g_signal_connect(G_OBJECT(timeline_renderer_text_final), "edited", G_CALLBACK(timeline_edited_final), NULL);

	// Hook a signal handler to the x offset column renderer
	g_signal_connect(G_OBJECT(timeline_renderer_text_x_offset), "edited", G_CALLBACK(timeline_edited_x_offset), NULL);

	// Hook a signal handler to the y offset column renderer
	g_signal_connect(G_OBJECT(timeline_renderer_text_y_offset), "edited", G_CALLBACK(timeline_edited_y_offset), NULL);

	// Hook a signal handler to the text data column renderer
	g_signal_connect(G_OBJECT(timeline_renderer_text_text), "edited", G_CALLBACK(timeline_edited_text), NULL);
	...
	// Create final column
	timeline_column_final = gtk_tree_view_column_new_with_attributes("Final", timeline_renderer_text_final, "text", TIMELINE_FINAL, "editable", TRUE, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(slide_data->timeline_widget), timeline_column_final);

	// Create X offset column
	timeline_column_x_off = gtk_tree_view_column_new_with_attributes("X Offset", timeline_renderer_text_x_offset, "text", TIMELINE_X_OFF, "editable", TRUE, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(slide_data->timeline_widget), timeline_column_x_off);

	// Create Y offset column
	timeline_column_y_off = gtk_tree_view_column_new_with_attributes("Y Offset", timeline_renderer_text_y_offset, "text", TIMELINE_Y_OFF, "editable", TRUE, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(slide_data->timeline_widget), timeline_column_y_off);

	// Create text data display column
	timeline_column_text = gtk_tree_view_column_new_with_attributes("Text", timeline_renderer_text_text, "text", TIMELINE_TEXT, "editable", TRUE, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(slide_data->timeline_widget), timeline_column_text);
...
gboolean display_dialog_highlight(layer *tmp_layer, gchar *dialog_title)
{
	...
	gtk_spin_button_set_value(GTK_SPIN_BUTTON(finish_button), tmp_layer->final_frame);
	...
	tmp_layer->final_frame = (guint) gtk_spin_button_get_value(GTK_SPIN_BUTTON(finish_button));
	...
	switch (tmp_layer->object_type)
	{
		case TYPE_GDK_PIXBUF:
			...
			if (TRUE == return_code)
			{
				...
				gtk_list_store_set(((slide *) current_slide->data)->layer_store, tmp_iter,
							TIMELINE_X_OFF, tmp_image_ob->x_offset_start,
							TIMELINE_Y_OFF, tmp_image_ob->y_offset_start,
							TIMELINE_START, tmp_layer->start_frame,
							TIMELINE_FINAL, tmp_layer->final_frame,
							-1);
			}
			break;
		...
		case TYPE_TEXT:
			...
			if (TRUE == return_code)
			{
				...
				gtk_list_store_set(((slide *) current_slide->data)->layer_store, tmp_iter,
							...
							TIMELINE_X_OFF, tmp_text_ob->x_offset_start,
							TIMELINE_Y_OFF, tmp_text_ob->y_offset_start,
							TIMELINE_START, tmp_layer->start_frame,
							TIMELINE_FINAL, tmp_layer->final_frame,
							TIMELINE_TEXT, gtk_text_buffer_get_text(((layer_text *) tmp_layer->object_data)->text_buffer, &text_start, &text_end, FALSE),
							-1);
			}
			break;
		...
		case TYPE_HIGHLIGHT:
			...
			if (TRUE == return_code)
			{
				...
				tmp_iter = tmp_layer->row_iter;
				gtk_list_store_set(((slide *) current_slide->data)->layer_store, tmp_iter,
							TIMELINE_X_OFF, tmp_highlight_ob->x_offset_start,
							TIMELINE_Y_OFF, tmp_highlight_ob->y_offset_start,
							TIMELINE_START, tmp_layer->start_frame,
							TIMELINE_FINAL, tmp_layer->final_frame,
							-1);
			}
...
void layer_new_highlight(void)
{
	...
	tmp_layer->final_frame = slide_length;
	...
	layer_pointer = ((slide *) current_slide->data)->layers;
	layer_pointer = g_list_first(layer_pointer);
	g_list_prepend(layer_pointer, tmp_layer);
...
void layer_new_text(void)
{
	...
	gtk_list_store_set(((slide *) current_slide->data)->layer_store, tmp_iter,
						...
						TIMELINE_X_OFF, tmp_text_ob->x_offset_start,
						TIMELINE_Y_OFF, tmp_text_ob->y_offset_start,
						TIMELINE_TEXT, gtk_text_buffer_get_text(tmp_text_ob->text_buffer, &text_start, &text_end, FALSE),
						-1);
...
void menu_help_about(void)
{
	...
	gtk_about_dialog_set_copyright(about_dialog, "(C) 2005-2006 Justin Clift");
