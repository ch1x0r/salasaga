...
gboolean display_dialog_text(layer *tmp_layer, gchar *dialog_title)
{
	...
	// Create the dialog
	text_dialog = GTK_DIALOG(gtk_dialog_new_with_buttons(dialog_title, GTK_WINDOW(get_main_window()), GTK_DIALOG_MODAL, GTK_STOCK_CANCEL, GTK_RESPONSE_REJECT, GTK_STOCK_OK, GTK_RESPONSE_ACCEPT, NULL));
	...
	gtk_table_attach(GTK_TABLE(appearance_table), GTK_WIDGET(text_frame), 0, 2, row_counter, row_counter + 1, GTK_EXPAND | GTK_FILL, GTK_EXPAND | GTK_FILL, table_padding_x, table_padding_y);
	...
	gtk_entry_set_max_length(GTK_ENTRY(name_entry), get_valid_fields_max_value(LAYER_NAME));
	...
	gtk_table_attach(GTK_TABLE(appearance_table), GTK_WIDGET(font_size_label), 0, 1, row_counter, row_counter + 1, GTK_EXPAND | GTK_FILL, GTK_FILL, table_padding_x, table_padding_y);

	// Create the entry that accepts the font size input
	font_size_scale = gtk_hscale_new_with_range(get_valid_fields_min_value(FONT_SIZE), get_valid_fields_max_value(FONT_SIZE), 0.1);
	...
	gtk_table_attach(GTK_TABLE(appearance_table), GTK_WIDGET(font_size_scale), 1, 2, row_counter, row_counter + 1, GTK_EXPAND | GTK_FILL, GTK_FILL, table_padding_x, table_padding_y);
	row_counter = row_counter + 1;
	...
	gtk_table_attach(GTK_TABLE(appearance_table), GTK_WIDGET(fg_colour_label), 0, 1, row_counter, row_counter + 1, GTK_EXPAND | GTK_FILL, GTK_FILL, table_padding_x, table_padding_y);
	...
	gtk_table_attach(GTK_TABLE(appearance_table), GTK_WIDGET(fg_colour_button), 1, 2, row_counter, row_counter + 1, GTK_EXPAND | GTK_FILL, GTK_FILL, table_padding_x, table_padding_y);
	row_counter = row_counter + 1;
	...
	border_width_scale = gtk_hscale_new_with_range(get_valid_fields_min_value(LINE_WIDTH), get_valid_fields_max_value(LINE_WIDTH), 0.1);
	gtk_range_set_value(GTK_RANGE(border_width_scale), tmp_text_ob->bg_border_width);
	gtk_scale_add_mark(GTK_SCALE(border_width_scale), 1.0, GTK_POS_BOTTOM, NULL);
	gtk_scale_add_mark(GTK_SCALE(border_width_scale), 2.0, GTK_POS_BOTTOM, NULL);
	...
	gtk_table_attach(GTK_TABLE(appearance_table), GTK_WIDGET(display_bg_checkbox), 1, 2, row_counter, row_counter + 1, GTK_EXPAND | GTK_FILL, GTK_FILL, table_padding_x, table_padding_y);
	...
	gtk_table_attach(GTK_TABLE(appearance_table), GTK_WIDGET(visibility_checkbox), 0, 1, row_counter, row_counter + 1, GTK_EXPAND | GTK_FILL, GTK_FILL, table_padding_x, table_padding_y);
	row_counter = row_counter + 1;
	...
	if (0 != get_debug_level())
	{
		...
		gtk_table_attach(GTK_TABLE(appearance_table), GTK_WIDGET(dump_buffer_label), 0, 1, row_counter, row_counter + 1, GTK_EXPAND | GTK_FILL, GTK_FILL, table_padding_x, table_padding_y);

		// Create the button to dump the text layer to stdout
		dump_buffer_button = gtk_button_new_with_label("Press to dump");
		gtk_table_attach(GTK_TABLE(appearance_table), GTK_WIDGET(dump_buffer_button), 1, 2, row_counter, row_counter + 1, GTK_EXPAND | GTK_FILL, GTK_FILL, table_padding_x, table_padding_y);
		row_counter = row_counter + 1;
	...
	start_scale = gtk_hscale_new_with_range(get_valid_fields_min_value(LAYER_DURATION), get_valid_fields_max_value(LAYER_DURATION), 0.1);
	gtk_range_set_value(GTK_RANGE(start_scale), tmp_layer->start_time);
	for (scale_mark_counter = 10; scale_mark_counter <= get_valid_fields_max_value(LAYER_DURATION); scale_mark_counter += 10.0)
	{
		// Add scale marks
	...
	scale_trans_in_duration = gtk_hscale_new_with_range(get_valid_fields_min_value(TRANSITION_DURATION), get_valid_fields_max_value(TRANSITION_DURATION), 0.01);
	gtk_range_set_value(GTK_RANGE(scale_trans_in_duration), tmp_layer->transition_in_duration);
	for (scale_mark_counter = 0.5; scale_mark_counter <= get_valid_fields_max_value(TRANSITION_DURATION); scale_mark_counter += 0.5)
	{
		// Add scale marks
	...
	duration_scale = gtk_hscale_new_with_range(get_valid_fields_min_value(LAYER_DURATION), get_valid_fields_max_value(LAYER_DURATION), 0.1);
	...
	scale_trans_out_duration = gtk_hscale_new_with_range(get_valid_fields_min_value(TRANSITION_DURATION), get_valid_fields_max_value(TRANSITION_DURATION), 0.01);
