...
gboolean display_dialog_highlight(layer *tmp_layer, gchar *dialog_title)
{
	...
	// Create the dialog
	highlight_dialog = GTK_DIALOG(gtk_dialog_new_with_buttons(dialog_title, GTK_WINDOW(get_main_window()), GTK_DIALOG_MODAL, GTK_STOCK_CANCEL, GTK_RESPONSE_REJECT, GTK_STOCK_OK, GTK_RESPONSE_ACCEPT, NULL));
	...
	gtk_box_pack_start(GTK_BOX(highlight_dialog->vbox), GTK_WIDGET(notebook_widget), TRUE, TRUE, 0);
	...
	gtk_range_set_value(GTK_RANGE(border_width_scale), tmp_highlight_ob->border_width);
	...
	opacity_scale = gtk_hscale_new_with_range(get_valid_fields_min_value(OPACITY), get_valid_fields_max_value(OPACITY), 0.1);
	gtk_range_set_value(GTK_RANGE(opacity_scale), tmp_highlight_ob->opacity);
	for (scale_mark_counter = 10; scale_mark_counter <= get_valid_fields_max_value(OPACITY); scale_mark_counter += 10)
	{
		// Add scale marks
	...
	width_scale = gtk_hscale_new_with_range(get_valid_fields_min_value(HIGHLIGHT_WIDTH), get_project_width(), 1);
	...
	height_scale = gtk_hscale_new_with_range(get_valid_fields_min_value(HIGHLIGHT_HEIGHT), get_project_height(), 1);
	...
	gtk_window_get_size(GTK_WINDOW(highlight_dialog), &dialog_width, &dialog_height);
	gtk_window_resize(GTK_WINDOW(highlight_dialog), (gint) roundf(dialog_width * 2.2), dialog_height);
	...
	do
	{
		...
		gfloat_val = gtk_range_get_value(GTK_RANGE(opacity_scale));
		...
		guint_val = gtk_range_get_value(GTK_RANGE(width_scale));
		...
		guint_val = gtk_range_get_value(GTK_RANGE(height_scale));
